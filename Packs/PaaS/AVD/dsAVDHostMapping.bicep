...
var dsArguments = '-Environment ${environment().name} -TenantId ${subscription().tenantId} -resourceGroup ${resourceGroup().name} -avdLogAlertsUri ${avdLogAlertsUri} -templateUri ${templateUri} -Tags """${Tags}""" -AGId ${AGId} -modulePrefix ${moduleprefix} -packtag ${packtag} -workspaceId ${workspaceId} -location ${location}'
