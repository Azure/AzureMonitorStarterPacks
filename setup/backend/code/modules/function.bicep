...
resource deploymentScript 'Microsoft.Resources/deploymentScripts@2020-10-01' = {
  ...
  properties: {
    ...
    scriptContent: 'echo "$CONTENT" > ${tempfilename} && cat ${tempfilename} | base64 -d > ${filename} && if [ az storage blob exists -c ${discoveryContainerName} -n ${filename} ]; storage blob delete -c ${discoveryContainerName} -n ${filename};fi && az storage blob upload -f ${filename} -c ${discoveryContainerName} -n ${filename} --overwrite true'
  }
}
