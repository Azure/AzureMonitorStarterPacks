...
resource deploymentScript 'Microsoft.Resources/deploymentScripts@2020-10-01' = {
  ...
  properties: {
    ...
    scriptContent: 'echo "$CONTENT" > ${tempfilename} && cat ${tempfilename} | base64 -d > ${filename} && az storage blob delete -f ${filename} -c ${discoveryContainerName} -n ${filename} && az storage blob upload -f ${filename} -c ${discoveryContainerName} -n ${filename} --overwrite true'
  }
}
